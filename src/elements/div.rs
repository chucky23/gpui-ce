...
impl Interactivity {
    ...
    /// Bind the given callback to magnification (pinch-to-zoom) events during the bubble phase.
                    ...
                    let element_transform = {
                        ...
                        if has_scale || has_rotation {
                            ...
                            if let Some(angle) = style.rotation {
                                ...
                                let visual_cx = bounds.origin.x.0 * sf
                                    + (cx_val - bounds.origin.x.0 * sf) * visual_scale;
                                let visual_cy = bounds.origin.y.0 * sf
                                    + (cy_val - bounds.origin.y.0 * sf) * visual_scale;
